...
def scale_final_reward(df, output_dir):
    ...
    # --- NEW: Clip extreme rewards before scaling ---
    # Calculate reward statistics before clipping
    orig_reward_min = df['final_reward'].min()
    orig_reward_max = df['final_reward'].max()
    orig_reward_mean = df['final_reward'].mean()
    orig_reward_std = df['final_reward'].std()
    print(f"Original reward stats: Min={orig_reward_min:.4f}, Max={orig_reward_max:.4f}, Mean={orig_reward_mean:.4f}, Std={orig_reward_std:.4f}")
    
    # Clip to +/- 5 standard deviations from mean to control outliers
    clip_min = orig_reward_mean - 5 * orig_reward_std
    clip_max = orig_reward_mean + 5 * orig_reward_std
    print(f"Clipping rewards to range: [{clip_min:.4f}, {clip_max:.4f}]")
    df['final_reward_clipped'] = df['final_reward'].clip(lower=clip_min, upper=clip_max) 

    # Calculate statistics after clipping
    clipped_count = ((df['final_reward'] < clip_min) | (df['final_reward'] > clip_max)).sum()
    print(f"Clipped {clipped_count} extreme reward values ({clipped_count/len(df):.2%} of data)")
    print(f"Reward stats after clipping: Min={df['final_reward_clipped'].min():.4f}, Max={df['final_reward_clipped'].max():.4f}")
    # Use the clipped rewards for scaling
    rewards = df[['final_reward_clipped']].astype(np.float32) # Use clipped rewards
    # --- End reward clipping ---    
    ...
    if rewards.isnull().any().any():
        ...
        df['final_reward_clipped'] = rewards['final_reward_clipped']
...
def construct_states(df):
    ...
    # --- ADDED: Clean NaN/Inf values in all columns ---
    # First check for NaNs/Infs
    nans_before = states_df.isnull().sum().sum()
    infs_before = (~np.isfinite(states_df)).sum().sum()
    
    if nans_before > 0 or infs_before > 0:
        print(f"Found {nans_before} NaNs and {infs_before} Infs in state features. Cleaning...")
        
        # Replace NaNs with 0.0 as a safe imputation (for non-lagged features)
        states_df = states_df.fillna(0.0)
        
        # Replace Infs with large but finite values
        states_df = states_df.replace([np.inf, -np.inf], [1.0e6, -1.0e6])
        
        # Check if cleaning worked
        nans_after = states_df.isnull().sum().sum()
        infs_after = (~np.isfinite(states_df)).sum().sum()
        print(f"After cleaning: {nans_after} NaNs and {infs_after} Infs remain.")
    # --- End cleaning ---
    ...
    if np.isnan(states).any() or np.isinf(states).any():
        # Final sanity check and emergency cleanup
        print("CRITICAL: NaNs or Infs still found after cleaning! Performing emergency replacement...")
        states = np.nan_to_num(states, nan=0.0, posinf=1.0e6, neginf=-1.0e6)
        
        # One more check (this should now be clean)
        if np.isnan(states).any() or np.isinf(states).any():
            raise ValueError("Failed to clean NaN/Inf values from state vectors! Training will fail.")
        else:
...
def extract_actions_terminals(df):
    ...
    epsilon = 1e-7 # Re-add epsilon addition
    actions += epsilon
...
def save_dataset(df, output_dir, output_filename, algo_type):
    ...
    # action_cols defined above - Action stats no longer saved
    print("Calculating reward statistics and max_timestep...") # Updated print
    ...
    # print(f"  Action Minimums: {action_min_vals}") # Removed
    # print(f"  Action Maximums: {action_max_vals}") # Removed
    print(f"  Reward Minimum: {reward_min_val:.4f}")
    print(f"  Reward Maximum: {reward_max_val:.4f}")
    
    stats_path = os.path.join(output_dir, "timestep_stats.npz") # Renamed file again
    print(f"Saving max_timestep to {stats_path}...") # Updated print: Only saving max_timestep
    try:
        # Save max_timestep along with action and reward stats
        np.savez(stats_path, # Updated path
                 # action_minimum=action_min_vals, # Removed
                 # action_maximum=action_max_vals, # Removed
                 # reward_minimum=np.float32(reward_min_val), # Removed
                 # reward_maximum=np.float32(reward_max_val), # Removed
                 max_timestep=max_timestep)
        print("Max timestep saved successfully.") # Updated print
    except Exception as e:
    ...
    if 'final_reward_scaled' not in df_filtered.columns:
        raise ValueError("Missing 'final_reward_scaled' column needed for DT dataset rewards.")
    rewards = df_filtered['final_reward_scaled'].astype(np.float32).to_numpy() # USE SCALED REWARDS
    print(f"Using 'final_reward_scaled' for dataset rewards. Mean: {rewards.mean():.4f}, Std: {rewards.std():.4f}")
...
def main():
    ...
    # print(f"Static Features Directory: {args.static_features_dir}") # Print static features dir
    ...
    try:
        ...
        # static_features_dict = load_static_features(args.static_features_dir)
        ...
        for raw_df_chunk in file_generator:
            file_count += 1
            print(f"\n  Processing file {file_count}...") # Changed print
            try:
                # --- Removed Static Feature Merge from here ---
